CREATE OR REPLACE VIEW full_view (play_id, track_id, track_name, track_pop, duration_ms, explicit, 
             ...
             artist_id, artist_name, date_time, pct_played)
    AS SELECT p.id play_id, p.track_id track_id, t.name track_name, t.popularity track_pop, duration_ms, explicit,
              alb.id album_id, alb.name album_name, album_type, total_tracks, release_date,
              art.id artist_id, art.name artist_name, date_time, pct_played
    FROM plays p
    INNER JOIN tracks t
    ON p.track_id = t.id
